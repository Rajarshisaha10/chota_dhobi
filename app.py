...
try:
    ...
    OCR_FEATURES_AVAILABLE = False

    def process_schedule_image(*_args, **_kwargs):
        raise NotImplementedError("OCR features are not available on this deployment.")

    def process_schedule_pdf(*_args, **_kwargs):
        raise NotImplementedError("OCR features are not available on this deployment.")

    def allowed_image(*_args, **_kwargs):
        return False

    def cleanup_file(*_args, **_kwargs):
        return None

    def extract_token_number(*_args, **_kwargs):
        raise NotImplementedError("OCR features are not available on this deployment.")

    def save_temp_upload(*_args, **_kwargs):
        raise NotImplementedError("OCR features are not available on this deployment.")

    def delete_image_if_exists(*_args, **_kwargs):
        return None

    def mark_lost_item_found(*_args, **_kwargs):
        return None

    def save_lost_found_image(*_args, **_kwargs):
        raise NotImplementedError("Lost-and-found image processing is not available.")
...
from vtop.profile_parser import parse_profile
import requests
...
db_dir = os.environ.get('SQLITE_DIR') or os.path.join(basedir, 'data')
os.makedirs(db_dir, exist_ok=True)
sqlite_main = 'sqlite:///' + os.path.join(db_dir, 'student.db')
app.config['SQLALCHEMY_DATABASE_URI'] = sqlite_main
...
def _extract_token_from_image_file(image):
    if not OCR_FEATURES_AVAILABLE:
        raise ValueError("OCR features are disabled in this deployment.")
...
def upload_schedule_image():
    if not OCR_FEATURES_AVAILABLE:
        return jsonify({"error": "OCR features are disabled in this deployment."}), 501
...
def generate_student_token_from_ocr():
    ...
    if not OCR_FEATURES_AVAILABLE and not manual_token:
        return jsonify({"error": "OCR features are disabled in this deployment.", "allowManualEntry": True}), 501
...
def report_lost_item():
    if not OCR_FEATURES_AVAILABLE:
        return jsonify({"error": "Lost-and-found image processing is disabled in this deployment."}), 501
    form = request.form
    try:
        student_id = _parse_int_field(form.get('studentId'), 'studentId')
